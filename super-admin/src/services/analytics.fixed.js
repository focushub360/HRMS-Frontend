...
export const analyticsService = {
  ...
  getDashboardAnalytics: async () => {
    ...
    try {
      ...
      // If the combined endpoint times out or is unreachable, try assembling
      // data from the individual endpoints so the UI can still render partial
      // analytics instead of failing outright.
      const isTimeout = err?.code === 'ECONNABORTED' || (err?.message || '').toLowerCase().includes('timeout');
      if (isTimeout) {
        console.warn('Combined analytics request timed out — falling back to individual endpoints.');
        try {
          return await assembleFromPieces();
        } catch (e) {
          // If assembling also fails, throw the original timeout error so callers
