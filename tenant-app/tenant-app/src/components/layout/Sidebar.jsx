...
import { leaveService, permissionService } from '../../services/auth';
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  // Team lead and Admin pending counts
  const [pendingCount, setPendingCount] = useState(0);
  const [adminLeavesCount, setAdminLeavesCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const loadPendingCounts = async () => {
      ...
      try {
        if (user?.role === 'teamlead') {
          ...
          setAdminLeavesCount(Array.isArray(leavesResp) ? leavesResp.length : 0);
          setAdminPermsCount(Array.isArray(permsResp) ? permsResp.length : 0);
        }
      } catch (error) {
    ...
    intervalId = setInterval(loadPendingCounts, 30000);
    
    // Refresh on window focus
    const handleFocus = () => loadPendingCounts();
  ...
  const getMenuItemIcon = (iconName, isActive = false) => {
    const iconClass = `w-5 h-5 ${isActive ? 'text-primary-600 dark:text-primary-300' : 'text-gray-500 dark:text-gray-300'}`;
          ...
          >
            <div className="flex items-center flex-1 relative">
              ...
              {user?.role === 'admin' && 
               ...
               adminLeavesCount > 0 && (
                <div className={`ml-2 px-2 py-1 rounded-full text-xs font-bold bg-red-500 text-white flex items-center shadow-sm transition-all duration-200 hover:scale-105 ${
                  adminLeavesCount > 9 ? 'animate-pulse' : ''
                }`}>
                  {adminLeavesCount > 99 ? '99+' : adminLeavesCount}
                </div>
              )}
              ...
              {user?.role === 'admin' && 
               ...
               adminPermsCount > 0 && (
                <div className={`ml-2 px-2 py-1 rounded-full text-xs font-bold bg-red-500 text-white flex items-center shadow-sm transition-all duration-200 hover:scale-105 ${
                  adminPermsCount > 9 ? 'animate-pulse' : ''
                }`}>
                  {adminPermsCount > 99 ? '99+' : adminPermsCount}
                </div>
              )}
            </div>
            ...
            {item.children.length > 0 && (
              <ChevronRightIcon 
                className={`w-4 h-4 transition-transform duration-200 text-gray-400 ${
                  expandedItems[item.title] ? 'rotate-90 text-gray-600 dark:text-gray-300' : ''
                }`}
              />
            )}
          </div>
                ...
                return (
                  <div
                    ...
                    className={`
                        ...
                        ${isChildActive 
                          ? 'bg-primary-50 text-primary-600 border-l-2 border-primary-500 -ml-px dark:bg-primary-900 dark:text-primary-300 dark:border-primary-700' 
                  ...
                  >
                    <div className={`w-1.5 h-1.5 rounded-full mr-3 ${
                      isChildActive ? 'bg-primary-500' : 'bg-gray-300 dark:bg-gray-600'
                    }`} />
  ...
  <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200 bg-gradient-to-r from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 dark:border-gray-700 transition-colors">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100 truncate max-w-[120px] transition-colors">
                {companyInfo.name}
              </h1>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 dark:bg-gray-900 dark:border-gray-700">
          <button
            ...
            className="flex items-center w-full px-3 py-2.5 text-sm font-medium text-gray-700 rounded-lg transition-all duration-200 hover:bg-white hover:text-red-600 hover:shadow-sm border border-transparent hover:border-gray-200 dark:text-gray-100 dark:hover:bg-gray-800 dark:hover:text-red-400 dark:hover:shadow-none"
          >
            <div className="p-1.5 rounded-lg bg-gray-200 text-gray-600 mr-3 dark:bg-gray-700 dark:text-gray-200">
