...
const AddProgressModal = ({ isOpen, onClose, task: initialTask, onSuccess }) => {
  ...
  // When selectedTask changes, check if current user already has an update for today
  useEffect(() => {
    let mounted = true;
    const checkTodayUpdate = async () => {
      if (!selectedTask) {
        setExistingUpdate(null);
        // prefill progress from selected task if available
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Track your daily work and progress</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-gray-900">{taskDetails.title}</h4>
                  <p className="text-sm text-gray-600">{taskDetails.project?.name}</p>
                </div>
